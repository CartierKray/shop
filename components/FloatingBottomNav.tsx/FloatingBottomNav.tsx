...
const FloatingBottomNav = () => {
  ...
  return (
    <div className="block md:hidden z-50 bg-transparent fixed inset-x-0 bottom-0">
      <motion.nav
        animate={open ? "open" : "closed"}
        initial="closed"
        className="bg-white/75 backdrop-blur-sm text-black shadow-lg flex items-center justify-between absolute bottom-8 left-[50%] -translate-x-[50%]"
      >
...
const Link = ({ text, Icon }: { text: string; Icon: IconType }) => {
  return (
    <a
      href="#"
      rel="nofollow"
      className="text-sm w-12 hover:text-emerald-600 transition-colors flex flex-col gap-1 items-center"
    >
      <Icon />
...
const Menu = () => {
  return (
    <motion.div
      ...
      className="p-8 bg-white rounded-3xl shadow-lg absolute bottom-[125%] left-[50%] flex w-[calc(100vw_-_48px)] max-w-lg"
    >
...
const MenuLink = ({ text }: { text: string }) => {
  return (
    <motion.a
      ...
      href="#"
      rel="nofollow"
      className="text-sm hover:text-emerald-600 transition-colors flex items-center gap-2"
    >
      {text}
    </motion.a>
  );
